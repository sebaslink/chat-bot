...
TWILIO_WHATSAPP_FROM = os.getenv('TWILIO_WHATSAPP_FROM', '+15005550009')

# Número de prueba
NUMERO_PRUEBA = os.getenv("NUMERO_PRUEBA", "+15005550009")
...
def log_mensaje(numero_id, mensaje, status, twilio_sid=None, error_message=None):
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO mensajes_log (numero_id, numero, mensaje, estado, twilio_sid, error_message) 
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (numero_id, numero_telefono, mensaje, status, twilio_sid, error_message))
        conn.commit()
        conn.close()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                success = agregar_numero(nombre, apellido, numero_limpio, carrera, grupo_id_final)
                if success:
...
def descargar_plantilla_excel():
    ...
    try:
        ...
        flash('Error creando plantilla Excel', 'error')
        return redirect(url_for('index'))
